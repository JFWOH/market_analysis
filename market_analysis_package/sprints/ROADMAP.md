# ROADMAP — Programa de Evolução `market_analysis`
...
|---|---|---:|---|
| 18 | Desambiguação de MDD (total vs capital-at-risk) | 3-4 | `findings/sprint_18_mdd_dual.md` |
| 19 | Sensibilidade a custos (slippage + comissão) | 3-4 | `findings/sprint_19_cost_sensitivity.md` |
| 20 | Decomposição fatorial (alpha vs exposição) | 5-7 | `findings/sprint_20_factor_decomp.md` |
| 21 | Walk-forward com re-otimização honesta | 7-10 | `findings/sprint_21_walkforward_honest.md` |
| 22 | Bears não-canônicos (15+ cenários) | 5-7 | `findings/sprint_22_bears_complete.md` |

**Marco final**: `findings/MARCO_BLOCO_I.md` define se Bloco II prossegue com:
...
| Sprint | Tema | Dias | Saída crítica |
|---|---|---:|---|
| 23 | Audit log append-only com hash chain | 4-6 | `audit_log.py` |
| 24 | Kill switch + circuit breakers + watchdog | 3-5 | `risk_guard.py` |
...
| 33 | Empacotamento (.exe com pywebview) | 5-7 | `market_analysis.spec` + `.bat` |
...
3. Sistema dispara processo de simulação
   └─> SessionManager.spawn(config) -> session_id UUID
   └─> Backend redireciona para /live/<session_id>
...
# Ler sprints/sprint_18_metricas.md inteiro
